...
/* @var $searchModel sklad\models\search\GofraSearch */
/* @var $dataProvider yii\data\ActiveDataProvider */

$this->title = 'Тип гофры';
$this->params['breadcrumbs'][] = $this->title;
app()->formatter->nullDisplay = '&mdash;';
?>


<div class="modal fade create-update-gofra" tabindex="-1" role="dialog"
    ...
    ['class' => 'kartik\grid\ActionColumn',
        ...
        'buttons'=>[
            'update' => function ($url, $model,$key){return Html::button('<span class="fas fa-lg fa-pencil-alt" style="font-size:14px;"></span>',
                [
                    'class' => 'update-gofra btn btn-link',
                    'data-id' => $model->id,
...
//        'rowOptions' => ['class' => GridView::ALIGN_RIGHT],
        ...
        'panelBeforeTemplate' => '
            {toolbarContainer}
             <button class="btn btn-success create-gofra" style="float: right; margin-right: 2px">Добавить</button>
            {before}
        ',
        'panelTemplate' => '
            {panelBefore}
...
<script type="text/javascript">
 <?php ob_start() ?>
    ...
    $('body').on('click', '.update-gofra', function () {
        var id = $(this).data('id');
        $.ajax({
            type: 'get',
            url: '<?= yii\helpers\Url::to(["gofra/update"]) ?>',
            data: {id: id},
            success: function (res) {
                $('#create-update-form').html(res);
                $('.create-update-gofra').modal('show');
            },
            error: function (e) {
                console.log(e);
            }
    ...
    $('.create-gofra').click(function () {
        $.ajax({
            type: 'get',
            url: '<?= yii\helpers\Url::to(["gofra/create"]) ?>',
            success: function (res) {
                $('#create-update-form').html(res);
                $('.create-update-gofra').modal('show');
            },
    ...
    $('body').on('click', '#save-gofra-form', function (e) {
        e.preventDefault();
        var form = $('#save-gofra');
        ...
        $.ajax({
            ...
            success: function (res) {
                if (res.success === 1) {
                    $('.create-update-gofra').modal('hide');
                    $('#create-update-form').html(res.view);

                    $.pjax.reload({container: "#pjax-gofra"});
